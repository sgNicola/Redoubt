...
def flow_sample(
    ...
    log_density = {"scenario_name": [], "log_p": [], "inference_time":[]}
    with torch.no_grad(): 
        for batch_idx, hidden_feature in enumerate(dataloader):
            ...
            log_density["inference_time"].append(inference_time)
    df = pd.DataFrame(log_density)
    total_inference_time = sum(log_density["inference_time"])
    average_inference_time = total_inference_time / len(log_density["inference_time"])
...
def run(data_dir, flow_checkpoint_path):
    ...
    save_path = f"{save_dir}/op_inference_density.parquet"
    df.to_parquet(save_path, index=False)
    print("save successfully")
    
if __name__ == "__main__":
    data_dir = "/home/sgwang/PlanScope/planscope_dataset"
    # flow_checkpoint_path = '/home/sgwang/PlanScope/FM_model/plantf_flow_nopro-epoch=1899-val_loss=0.5131.ckpt'
    flow_checkpoint_path = '/home/sgwang/PlanScope/FM_model/pluto_flow-epoch=1889-val_loss=0.3266.ckpt'
    # flow_checkpoint_path = '/home/sgwang/PlanScope/FM_model/scope_flow_nopro-epoch=1709-val_loss=0.3610.ckpt'
