...
class LitSparseRegression(pl.LightningModule):
    ...
    def _compute_losses(self, outputs, targets):
        ...
        ade_weight = 1.0 / (loss_dict['ade'].detach() + 1e-6)
        collision_weight = 1.0 / (loss_dict['collision_cls'].detach() + 1e-6)
        drivable_weight = 1.0 / (loss_dict['drivable_cls'].detach() + 1e-6)
