...
def custom_collate_fn(batch):
    ...
    for item in batch:
        hidden_chunks = torch.chunk(item["hidden_feature"], 3, dim=0)
        ade_chunks = torch.chunk(item["ADE"].view(-1), 3, dim=0)
        collision_chunks = torch.chunk(item["collision_energy"], 3, dim=0)
        drivable_chunks = torch.chunk(item["drivable_score"], 3, dim=0)
