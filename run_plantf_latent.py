...
from src.metric_score.compute_metric import ComputeMetric
from src.models.pluto.loss.esdf_collision_loss import ESDFCollisionLoss
from src.models.planTF.inference_plantf import PlanningModel
import os
...
def main(cfg: DictConfig):
    ...
    device = 'cpu'
    plantf_dataset = os.getenv("PLANTF")   
    OT =80
    # radius = model.radius
    ...
    save_dir = f"{plantf_dataset}/{cfg.stage}_results"
    ...
    with torch.no_grad():
        for batch_index, batch in enumerate(dataloader):
            ...
            best_mode = res["probability"].argmax(dim=-1)
            best_trajectory = trajectory[torch.arange(bs), best_mode]
            ...
            save_path = f"{plantf_dataset}/{cfg.stage}_results/{batch_index}.pt"
            torch.save(
                results, save_path
            )
