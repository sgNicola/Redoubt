...
from src.feature_builders.plantf_common.utils import rotate_round_z_axis
...
class ImitationPlanner(AbstractPlanner):
    ...
    def _run_planning_once(self, current_input: PlannerInput):
        ...
        out, encoder_feature= self._planner.forward(planner_feature_torch.data)
        local_trajectory = out["output_trajectory"][0].cpu().numpy()
        pi = out["probability"].cpu().numpy()
        
        return local_trajectory.astype(np.float64), pi.flatten(), encoder_feature.detach().cpu()
    ...
    def compute_planner_trajectory(
        ...
        if self._last_plan_elapsed_step >= self._replan_interval:
            local_trajectory, _ , encoder_feature= self._run_planning_once(current_input)
            del encoder_feature
