...
@dataclass
class ScopeFeature(AbstractModelFeature):
    ...
    @classmethod
    def collate(cls, feature_list: List[ScopeFeature]) -> ScopeFeature:
        batch_data = {}
        ...
        return ScopeFeature(data=batch_data)

    def to_feature_tensor(self) -> ScopeFeature:
        new_data = {}
    ...
    def to_numpy(self) -> ScopeFeature:
        new_data = {}
        for k, v in self.data.items():
        ...
        if self.data_n is not None:
            ...
            new_data_n = None
        return ScopeFeature(data=new_data, data_p=new_data_p, data_n=new_data_n)

    def to_device(self, device: torch.device) -> ScopeFeature:
        new_data = {}
        for k, v in self.data.items():
            new_data[k] = to_device(v, device)
        return ScopeFeature(data=new_data)

    def serialize(self) -> Dict[str, Any]:
        return {"data": self.data}

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> ScopeFeature:
        return ScopeFeature(data=data["data"])
