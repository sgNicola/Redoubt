...
# same as common/metric_aggregator
...
# same as scenario_map in nuplan_scenario_mapping.yaml
...
# config in scenario_filter
def get_filter_parameters(config_path):
    with open(config_path, 'r') as file:
        config = yaml.safe_load(file)
    config = DictConfig(config)
    return {
        'scenario_types': config.scenario_types,
        'scenario_tokens': config.scenario_tokens,
        'log_names': config.log_names,
        'map_names': config.map_names,
        'num_scenarios_per_type': config.num_scenarios_per_type,
        'limit_total_scenarios': config.limit_total_scenarios,
        'timestamp_threshold_s': config.timestamp_threshold_s,
        'ego_displacement_minimum_m': config.ego_displacement_minimum_m,
        'expand_scenarios': config.expand_scenarios,
        'remove_invalid_goals': config.remove_invalid_goals,
        'shuffle': config.shuffle,
        'ego_start_speed_threshold': config.ego_start_speed_threshold,
        'ego_stop_speed_threshold': config.ego_stop_speed_threshold,
        'speed_noise_tolerance': config.speed_noise_tolerance
    }
