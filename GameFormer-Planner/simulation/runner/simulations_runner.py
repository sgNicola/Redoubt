...
from simulation.runner.runner_report import RunnerReport
from simulation.runner.abstract_runner import AbstractRunner
...
class SimulationRunner(AbstractRunner):
    ...
    def run(self) -> RunnerReport:
        ...
        report = RunnerReport(
            ...
            scenario_type=self._simulation.scenario.scenario_type,
        ...
        while self.simulation.is_simulation_running():
            ...
            trajectory, score, encoder_feature = self.planner.compute_trajectory(planner_input)
            # TODO: Add OOD score to the report, check the planner._planning method
            results["hidden_feature"].append(encoder_feature)
            score_numpy = score.detach().cpu().numpy().flatten() if score.requires_grad else score.cpu().numpy().flatten()
            report.risk_score.append(score_numpy)
        ...
        results['hidden_feature'] = torch.stack(results['hidden_feature']) 
        save_dir = f"{sim_data}/simulation_results"
        os.makedirs(save_dir, exist_ok=True)
        save_path = f"{save_dir}/{report.scenario_name}.pt"
