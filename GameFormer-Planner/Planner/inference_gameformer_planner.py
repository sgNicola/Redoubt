...
class Planner(AbstractPlanner):
    ...
    def _get_prediction(self, features):
        predictions, plan, hidden_feature = self._model(features)
        ...
        return plan, final_predictions, final_scores, ego_current, neighbors_current, hidden_feature.detach().cpu()
    ...
    def _plan(self, ego_state, history, traffic_light_data, observation):
        ...
        with torch.no_grad():
            plan, predictions, scores, ego_state_transformed, neighbors_state_transformed, hidden_feature = self._get_prediction(features)
        ...
        return trajectory, scores, hidden_feature
    ...
    def compute_planner_trajectory(self, current_input: PlannerInput):
        ...
        trajectory, scores, hidden_feature = self._plan(ego_state, history, traffic_light_data, observation)
