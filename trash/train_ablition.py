...
def custom_collate_fn(batch):
    ...
    all_hidden_features = [chunk for sample in batch for chunk in torch.chunk(sample, 3, dim=0)]  # List of [1, 128]
...
def train(resume, data_dir, model_name, ckpt_path=None):
    ...
    checkpoint_callback = ModelCheckpoint(
        monitor="val_loss",
        dirpath="./FM_model",
        filename=f"{model_name}_flow_nopro-{{epoch:03d}}-{{val_loss:.4f}}",
        save_top_k=3,
        mode="min",
        save_last=True,   
        every_n_epochs=10
